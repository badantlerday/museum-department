...
const Bookmark = ({ documentId, isBookmarked: initialIsBookmarked, userid, variant = "text", message }) => {
  ...
  const handleBookmarkClick = async () => {
    ...
    try {
      if (isBookmarked) {
        const { data, error } = await removeBookmark({ userid, docid: documentId });
  ...
  return (
      ...
      <button onClick={handleBookmarkClick} disabled={isLoading} className='flex items-center'>
        {variant === 'text' && (
          isLoading ? 'Processing...' : isBookmarked ? 'Remove Bookmark' : 'Add Bookmark'
        )}
        {variant === 'icon' && (
          isLoading ? (
            
                <svg class="animate-spin h-[15px] w-[15px] text-md-black" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
