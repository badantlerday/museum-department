...
export default function CurrentPageTitle({ data }) {
  ...
  useEffect(() => {
    if (pathname === "/") {
      ...
      if (matchedItem) {
        if (
          matchedItem.type === "category" ||
          matchedItem.type === "city" ||
          matchedItem.type === "country" ||
          matchedItem.type === "person"
        ) {
          setCurrentHeader(`Reference`);
        } else {
          setCurrentHeader(
            `${matchedItem.type.charAt(0).toUpperCase() + matchedItem.type.slice(1)} – ${matchedItem.name}`,
          );
        }
      } else {
  ...
  return (
    <div
      className="animate-fadeIn font-medium text-base tracking-[1%]"
      key={pathname}
    >
      {currentHeader}
    </div>
